...
frontier_q = ['https://www.ku.ac.th/th/']
# frontier_q = ['https://cpe.ku.ac.th/']
visited_q = []

NETLOC_CONSECUTIVE_LIMIT = 5
NETLOC_CONSECUTIVE_TIMEOUT_SEC = 30
netloc_consecutive_count = {}

EXCLUDED_EXTENSIONS = {
    '.jpg', '.jpeg', '.png', '.svg', '.gif', '.webp', '.bmp', '.tiff',  # Images
    '.pdf', '.doc', '.docx', '.ppt', '.pptx', '.xls', '.xlsx',  # Documents
    '.zip', '.rar', '.tar', '.gz', '.7z', '.iso',  # Archives
    '.mp3', '.wav', '.ogg', '.mp4', '.avi', '.mov',  # Media
    '.exe', '.dll', '.bin', '.bat', '.sh',  # Executables and scripts
    '.css', '.js', '.json', '.xml', '.csv', '.txt',  # Other common file types
}

async def wait_enable(netloc, timeout_sec):
    await asyncio.sleep(timeout_sec)
    netloc_consecutive_count[netloc] = 0

def enqueue(urls):
    for url in urls:
        url_parts = urlsplit(url)
        if (url not in frontier_q and
            url not in visited_q and
            url_parts.scheme.startswith('http') and
            url_parts.netloc.endswith('.ku.ac.th') and
            not url_parts.path.endswith(tuple(EXCLUDED_EXTENSIONS))
        ):
...
async def dequeue():
    ...
    for i in range(len(frontier_q)):
        ...
        if netloc_consecutive_count.get(netloc) is None:
            netloc_consecutive_count[netloc] = 0

        if netloc_consecutive_count[netloc] < NETLOC_CONSECUTIVE_LIMIT:
            current_url = frontier_q.pop(i)
            netloc_consecutive_count[netloc] += 1

        if netloc_consecutive_count[netloc] == NETLOC_CONSECUTIVE_LIMIT:
            asyncio.create_task(wait_enable(netloc, NETLOC_CONSECUTIVE_TIMEOUT_SEC))
            netloc_consecutive_count[netloc] += 1
    ...
    while len(frontier_q) > 0 and html_count < 10000:
        ...
        if current_url_parts.netloc not in netloc_seen:
            ...
            try:
                ...
                # below will only be executed if robots.txt exists
                rp = RobotFileParser()
        ...
        if (netloc_rp.get(current_url_parts.netloc) and
            not netloc_rp[current_url_parts.netloc].can_fetch(USER_AGENT, current_url)
        ):
            continue
        ...
        try:
            ...
            # write html to file
            path_part = current_url_parts.path.strip('/').replace('.htm', '.html')
